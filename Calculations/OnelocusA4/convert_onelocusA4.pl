...
$ofile1 = "Maxima/onelocusA4_maxima.txt";
open(OUT1, ">$ofile1") or die("Cannot write to $ofile1");
$ofile2 = "R/onelocusA4.R";
open(OUT2, ">$ofile2") or die("Cannot write to $ofile2");
print OUT1 "/* one autosomal locus, 4-way RIL by sib mating */\n";
print OUT1 "/*  to load, use: batch(\"$ofile1\"); */\n\n";
print OUT2 "# one autosomal locus, 4-way RIL by sib mating\n\n";

$ofile3 = "Latex/onelocusA4_tables.tex";
...
print OUT3 "\\textbf{\\sffamily One autosomal locus, 4-way RIL by sibling mating}\n\\bigskip\n\n";
...
print OUT3 "(where \$\\text{G}_2\\text{F}_0 \\equiv AB \\times CD\$)\\\\[6pt]\n";
...
$ifile = "Inputs/pi_k_ind.txt";
open(IN, $ifile) or die("Cannot read from $ifile");
$line = <IN>;
print OUT1 "/* individual probabilities */\n";
print OUT1 "indk : matrix([";
print OUT2 "# individual probabilities\n";
print OUT2 "indk <- function(k) c(";
$flag = 0;
while($line = <IN>) {
    if(!$flag) { $flag = 1; }
