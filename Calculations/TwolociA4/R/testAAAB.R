...
indprob <- lapply(indprob, function(a) a[,sub("\\|", ".", rownames(indAAABstart))])


######################################################################
# exact calculation of a subset of those, by matrix multiplication
######################################################################
...
for(i in seq(along=indprob2)) {
  ...
  colnames(indprob2[[i]]) <- colnames(indprob[[i]])
  indprob2[[i]][1,] <- as.numeric(pi0 %*% b)
  cur <- pi0
  for(j in 1:(nrow(indprob[[1]])-1)) {
    cur <- cur %*% Am
    indprob2[[i]][j+1,] <- as.numeric(cur %*% b)
  }
}

######################################################################
# exact calculation of a subset of those, by eigen decomposition
######################################################################
...
for(i in seq(along=indprob3)) {
  ...
  colnames(indprob3[[i]]) <- colnames(indprob[[i]])
  indprob3[[i]][1,] <- as.numeric(pi0 %*% b)
  for(j in 1:(nrow(indprob[[1]])-1)) {
    temp <- (V %*% diag(e^j) %*% Vi %*% b)
    indprob3[[i]][j+1,] <- as.numeric(pi0 %*% temp)
  }
}
